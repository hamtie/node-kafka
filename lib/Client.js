...
module.exports = std.Class(Connection, function(supr) {
    ...
    this.init = function(opts) {
        ...
        opts = helpers.extend(opts, defaults)
    ...
    this.fetchTopic = function(args) {
        var request = helpers.extend({}, helpers.extend(args.name == undefined ? { name: args } : args, fetch_defaults))
    ...
    this.fetchOffsets = function(args) {
        var request = helpers.extend({}, helpers.extend(args.name == undefined ? { name: args } : args, offset_defaults))
        this._pushRequest({request: request})
        return this
    }

    this.send = function(args, messages, callback) {
        var request = helpers.extend({}, helpers.extend(messages == undefined ? args : { topic: args, messages: messages}, send_defaults))
    ...
    this._encodeMessage = function(message) {
        return std.pack('CN', MAGIC_VALUE, std.crc32(message)) + std.utf8_encode(message)
    }
